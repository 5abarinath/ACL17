        ...
        connection.query(sql_team_members, ['2'], function(err2, result2) {
            ...
            connection.query(sql_team_members, ['3'], function(err3, result3) {
                ...
                connection.query(sql_team_members, ['4'], function(err4, result4) {
                    ...
                    //For Premier Titans (Aman KIA Motors)
...
app.post('/authenticate', function(req, res) {
    ...
    if ((user == 1 && password == "abc123") || (user == 2 && password == "abc123") || (user == 3 && password == "abc123") || (user == 4 && password == "abc123") || (user == 5 && password == "abc123") || (user == 6 && password == "abc123")) {
...
function commitPremiumsToDatabase() {
    redisClient.hget("aclteam1", 'premLeft', function(err, result1) {
        if (err) {
            console.log(err);
            throw err;
        }
        redisClient.hget("aclteam2", 'premLeft', function(err, result2) {
            if (err) {
                console.log(err);
                throw err;
            }
            redisClient.hget("aclteam3", 'premLeft', function(err, result3) {
                if (err) {
                    console.log(err);
                    throw err;
                }
                redisClient.hget("aclteam4", 'premLeft', function(err, result4) {
                    if (err) {
                        console.log(err);
                        throw err;
                    }
                    redisClient.hget("aclteam5", 'premLeft', function(err, result5) {
                        if (err) {
                            console.log(err);
                            throw err;
                        }
                        redisClient.hget("aclteam6", 'premLeft', function(err, result6) {
                            if (err) {
                                console.log(err);
                                throw err;
                            }
